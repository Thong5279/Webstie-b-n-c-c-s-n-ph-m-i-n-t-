        ...
        <div className=" p-6 bg-gray-50 rounded-lg shadow-md">
            ...
            {isVoucherValid && discountAmount > 0 && (
                <div className="mt-6 p-4 bg-green-100 rounded-lg border border-green-300">
                    <p className="text-lg text-green-700">Giảm giá: -{displayVNDCurrency(discountAmount)}</p>
                    <p className="text-xl font-bold text-green-800">Tổng tiền sau giảm: {displayVNDCurrency(totalDiscountAmount)}</p>
                </div>
            )}
        </div>
    );
